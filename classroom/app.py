    ...
    <body>
        <div class="container">
            ...
            <form id="uploadForm" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="comment_style">评语风格（仅在AI生成时生效）</label>
                    <select id="comment_style" name="comment_style">
                        <option value="professional">专业点评型（默认）</option>
                        <option value="warm">温暖鼓励型</option>
                        <option value="childlike">童趣活泼型</option>
                    </select>
                </div>
                ...
                <div class="tips" style="background:#e8f5e9; margin-bottom:15px;">
                    ...
                    • 选择"评语风格"可自定义AI点评的语气<br>
                    • 也可手动输入，系统将直接使用您的评语<br>
                    • 3种风格：专业（从技法分析），温暖（鼓励为主），活泼（儿童友好）
                </div>
...
def submit_record():
    ...
    try:
        ...
        comment_style = request.form.get("comment_style", "professional").strip()  # 获取风格，默认为preofessional
        posture = request.files["posture"]
        work = request.files["work"]

        # 2. 验证风格参数
        valid_styles = ["professional", "warm", "childlike"]
        if comment_style not in valid_styles:
            comment_style = "professional"
            logger.warning(f"⚠️ 验收到无效风格: {request.form.get('comment_style')}，默认为professional")
        ...
        if not comment or comment.strip() == "":
            ai_comment, ai_error, generation_time_ms = generate_ai_comment(
                work_path, student_name, style=comment_style
            )
            if ai_comment:
                comment = ai_comment
        ...
        record = {
            ...
            "comment_style": comment_style if ai_comment is not None else None,  # 仅当AI生成时记录风格
